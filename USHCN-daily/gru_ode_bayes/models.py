...
class GRUODECell_Autonomous_Attention(torch.nn.Module):
    def __init__(self, hidden_size, bias=True):
        """
        For p(t) modelling input_size should be 2x the x size.
        """
        ...
        self.lin_hz_at = torch.nn.Linear(hidden_size, hidden_size, bias=False)
        self.lin_hn_at = torch.nn.Linear(hidden_size, hidden_size, bias=False)
    ...
    def forward(self, t, h):
        ...
        hidden_size = self.hidden_size # 20 
        size = h.shape  # 500,20 
        size= int(size[0]) - hidden_size 
        
        h0 = h[:size] # 200,10 
        a0 = h[size:] # 10 10  
        
        a_s = torch.sigmoid(a0)
        h_ = torch.matmul(h0,a_s)

        x = torch.zeros_like(h_)
        z = torch.sigmoid(x + self.lin_hz(h_))
        n = torch.tanh(x + self.lin_hn(z * h_))

        xa = torch.zeros_like(h_)
        za = torch.sigmoid(xa + self.lin_hz_at(h_))
        na = torch.tanh(xa + self.lin_hn_at(za * h_))

        dh = (1 - z) * (n - h_) 
        dha = (1-za) * (na - h_)
        
        
        out = torch.cat([dh,dha])
        return out
...
class FullGRUODECell_Attention(torch.nn.Module):
    def __init__(self, input_size, hidden_size, bias=True):
        """
        For p(t) modelling input_size should be 2x the x size.
        """
        ...
        self.lin_x = torch.nn.Linear(input_size, hidden_size *500* 3, bias=bias)

        self.lin_hh = torch.nn.Linear(hidden_size*500, hidden_size*500, bias=False)
        self.lin_hz = torch.nn.Linear(hidden_size*500, hidden_size*500, bias=False)
        self.lin_hr = torch.nn.Linear(hidden_size*500, hidden_size*500, bias=False)

        self.lin_x_at = torch.nn.Linear(input_size, hidden_size *500* 3, bias=bias)

        self.lin_hh_at = torch.nn.Linear(hidden_size*500, hidden_size*500, bias=False)
        self.lin_hz_at = torch.nn.Linear(hidden_size*500, hidden_size*500, bias=False)
        self.lin_hr_at = torch.nn.Linear(hidden_size*500, hidden_size*500, bias=False)
    ...
    def forward(self, x, h):
        ...
        h0 = h[:size]
        a0 = h[size:]
        h0 = torch.reshape(h0,(1,25000)) 
        a0 = torch.reshape(a0,(1,25000))
        aT = torch.transpose(a0,0,1)
        a0 = torch.matmul(aT,a0) 
        a_s = torch.sigmoid(a0)
        h_ = torch.mul(h0,a_s)

        xr, xz, xh = torch.chunk(self.lin_x(x), 3, dim=1)

        r = torch.sigmoid(xr + self.lin_hr(h_))
        z = torch.sigmoid(xz + self.lin_hz(h_))
        u = torch.tanh(xh + self.lin_hh(r * h_))

        xra, xza, xha = torch.chunk(self.lin_x(x), 3, dim=1)
        
        ra = torch.sigmoid(xra + self.lin_hr(h_))
        za = torch.sigmoid(xza + self.lin_hz(h_))
        ua = torch.tanh(xha + self.lin_hh(ra * h_))

        dh = (1 - z) * (u - h_)
        dha = (1-za) * (ua - h_)
        dh = torch.reshape(dh,(500,50))
        dha = torch.reshape(dha,(500,50))
        out = torch.cat([dh,dha])
        return out 
...
class FullGRUODECell_Autonomous_Attention(torch.nn.Module):
    ...
    def __init__(self, hidden_size, bias=True):
        ...
        self.lin_hh_at = torch.nn.Linear(hidden_size, hidden_size, bias=False)
        self.lin_hz_at = torch.nn.Linear(hidden_size, hidden_size, bias=False)
        self.lin_hr_at = torch.nn.Linear(hidden_size, hidden_size, bias=False)

        self.softmax   = torch.nn.Softmax(dim=1) # softmax 
        # self.atten_init = torch.nn.Linear(hidden_size,hidden_size*hidden_size,bias=False)
    ...
    def forward(self, t, h):
        ...
        # import pdb
        # pdb.set_trace()
        hidden_size = self.hidden_size # 50 

        size= int(h.shape[0])
        size0 = size - hidden_size

        # unpack 
        h0 = h[:size0] # batch, hidden_size 
        a0 = h[size0:] # hidden_size, hidden_size
        
        a_s = self.softmax(a0)   
        # import pdb
        # pdb.set_trace()
        h0 = torch.transpose(h0,0,1)
        h_ = torch.matmul(a_s,h0)
        h_ = torch.transpose(h_,0,1)
        # co-evolving
        x = torch.zeros_like(h_)
        r = torch.sigmoid(x + self.lin_hr(h_))
        z = torch.sigmoid(x + self.lin_hz(h_))
        u = torch.tanh(x + self.lin_hh(r * h_))

        xa = torch.zeros_like(h_)
        ra = torch.sigmoid(xa + self.lin_hr_at(h_))
        za = torch.sigmoid(xa + self.lin_hz_at(h_))
        ua = torch.tanh(xa + self.lin_hh_at(ra*h_))

        dh = (1 - z) * (u - h_)
        dha = (1-za) * (ua - h_)
        
        out = torch.cat([dh,dha]) #concat 
        return out 
...
class GRUObservationCellLogvar(torch.nn.Module):
    ...
    def forward(self, h, p, X_obs, M_obs, i_obs):
        ...
        mean, logvar = torch.chunk(p_obs, 2, dim=1) #102,5 :: 102,5 
...
class NNFOwithBayesianJumps(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, p_hidden, prep_hidden, bias=True, cov_size=1, cov_hidden=1, classification_hidden=1, logvar=True, mixing=1, dropout_rate=0, full_gru_ode=False, solver="dopri5", impute = True, **options):
        """
    ...
    def ode_step(self, h, p, delta_t, current_time):
        ...
        if self.solver == "euler":
            ...
            # pdb.set_trace()
            #h : 500,30
            solution, eval_times = odeint_adjoint(self.gru_c,h,torch.tensor([0,delta_t]),method=self.solver,options={"store_hist":self.store_hist})
            # if self.store_hist:
...
class NNFOwithBayesianJumpsAttention(torch.nn.Module):
    ## Neural Negative Feedback ODE with Bayesian jumps
    def __init__(self, input_size, hidden_size, p_hidden, prep_hidden, bias=True, cov_size=1, cov_hidden=1, classification_hidden=1, logvar=True, mixing=1, dropout_rate=0, full_gru_ode=False, solver="euler", impute = True,batch=150, **options):
        """
        The smoother variable computes the classification loss as a weighted average of the projection of the latents at each observation.
        impute feeds the parameters of the distribution to GRU-ODE at each step.
        """
        ...
        if full_gru_ode:
            if impute is False:
                self.gru_c   = FullGRUODECell_Autonomous_Attention(hidden_size, bias = bias)
            ...
            if impute is False:
                self.gru_c = GRUODECell_Autonomous_Attention(hidden_size, bias= bias)
            else:
                self.gru_c   = GRUODECell(2 * input_size, hidden_size, bias=bias)
    ...
    def ode_step(self, h, p, delta_t, current_time):
        ...
        if self.solver == "euler":
            ...
            solution = self.gru_c(p,h) 
            sol_size = int(solution.shape[0]/2)
            h = h[:sol_size]
            #create attention map from h  
            input = h.cpu().detach().numpy()
            inputT = np.transpose(input)
            corr = np.corrcoef(inputT)
            a0 = torch.Tensor(corr)
            a =  a0.cuda()
            h = h + delta_t * solution[:sol_size]    
        ...
        current_time += delta_t
        return h,a,p,current_time, eval_times, eval_ps

        raise ValueError(f"Unknown solver '{self.solver}'.")
  
    def forward(self, times, time_ptr, X, M, obs_idx, delta_t, T, cov,
                return_path=False, smoother = False, class_criterion = None, labels=None,plot = False):
        """
        Args:
            times      np vector of observation times
            time_ptr   start indices of data for a given time
            ...
            cov        static covariates for learning the first h0
            return_path   whether to return the path of h
            a          make attention vector from h 
        ...
        h = self.covariates_map(cov) # batch, hidden_size 
        #create attention map from h 
        ...
        for i, obs_time in enumerate(times):
            ...
            while current_time < (obs_time-0.001*delta_t): #0.0001 delta_t used for numerical consistency.
                ...
                if self.solver == "dopri5":
                    h, a,p, current_time, eval_times, eval_ps = self.ode_step(x0, p, obs_time-current_time, current_time)
                else:
                    
                    h,a, p, current_time, eval_times, eval_ps = self.ode_step(x0, p, delta_t, current_time)
            ...
            # create attention map from updated h 
            input = h.cpu().detach().numpy()
            inputT = np.transpose(input)
            corr = np.corrcoef(inputT)
            a0 = torch.Tensor(corr)
            ...
            # add l2 regularity for attention map
            import pdb 
            # pdb.set_trace() 
            theta_g = torch.norm(a, p = 2) #l2
            loss_2_org = loss_2 + compute_KL_loss(p_obs = p[i_obs], X_obs = X_obs, M_obs = M_obs, logvar=self.logvar)
            loss_2 = loss_2_org + 0.1*theta_g 
        ...
        while current_time < T:
            ...
            if self.solver == "dopri5":
                h, a,p, current_time,eval_times, eval_ps = self.ode_step(x0, p, T-current_time, current_time)    
            else:
                h, a,p, current_time,eval_times, eval_ps = self.ode_step(x0, p, delta_t, current_time)
            eval_times_total = torch.cat((eval_times_total,eval_times))
            eval_vals_total  = torch.cat((eval_vals_total, eval_ps))
