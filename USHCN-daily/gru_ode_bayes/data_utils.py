...
def extract_from_path(t_vec, p_vec, eval_times, path_idx_eval):
    ...
    t_vec, unique_index = np.unique(t_vec,return_index=True) #1919
    p_vec = p_vec[unique_index,:,:] #1919,110,10 ->1694,110,10
...
def extract_from_path_plot(t_vec, p_vec, eval_times, path_idx_eval):
    '''
    Takes :
    t_vec : numpy vector of absolute times length [T]. Should be ordered.
    p_vec : numpy array of means and logvars of a trajectory at times t_vec. [T x batch_size x (2xfeatures)]
    ...
    return(p_vec[time_idx,path_idx_eval,:],eval_times)
