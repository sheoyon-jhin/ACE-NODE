...
parser.add_argument('--p_hidden', type = int, default = 25 )
parser.add_argument('--prep_hidden', type = int, default = 20 )
...
#'/home/bigdyl/gru_ode_bayes/experiments/Climate'
import pdb
# pdb.set_trace()
...
def train_gruode(simulation_name,params_dict,device, train_idx, val_idx, test_idx, epoch_max=40):
    ...
    dl   = DataLoader(dataset=data_train, collate_fn=data_utils.custom_collate_fn, shuffle=True, batch_size=params_dict["batch"],num_workers=1)
    ...
    # np.save(f"/home/bigdyl/gru_ode_bayes/experiments/Climate/trained_models_atten/{simulation_name}_params.npy",params_dict)
    np.save(ROOT_PATH+f"/trained_models_atten/{simulation_name}_params.npy",params_dict)


    nnfwobj = gru_ode_bayes.NNFOwithBayesianJumpsAttention(input_size = params_dict["input_size"], hidden_size = params_dict["hidden_size"],
                                            ...
                                            dropout_rate = params_dict["dropout_rate"],full_gru_ode= params_dict["full_gru_ode"], impute = params_dict["impute"],batch = params_dict["batch"])
    nnfwobj.to(device)
    ...
    for epoch in range(epoch_max):
        ...
        for i, b in enumerate(tqdm.tqdm(dl)):
            ...
            total_loss = (loss + params_dict["lambda"]*class_criterion(class_pred[:labels.shape[0]], labels))/batch_size
        ...
        with torch.no_grad():
            ...
            if val_metric > val_metric_prev:
                ...
                print("Saving Model")
                # torch.save(nnfwobj.state_dict(),f"/home/bigdyl/gru_ode_bayes/experiments/Climate/trained_models_atten/{simulation_name}_MAX.pt")
                torch.save(nnfwobj.state_dict(),ROOT_PATH+f"/trained_models_atten/{simulation_name}_MAX.pt")
                ...
                print(f"DD"+"Test loglik loss at epoch {epoch} : {test_loglik}")
                print(f"Test AUC loss at epoch {epoch} : {test_auc}")
                print(f"Test MSE loss at epoch{epoch} : {test_mse}")
            else:
                if epoch % 10:

                    # torch.save(nnfwobj.state_dict(),f"/home/bigdyl/gru_ode_bayes/experiments/Climate/trained_models_atten/{simulation_name}.pt")
                    torch.save(nnfwobj.state_dict(),ROOT_PATH+f"/trained_models_atten/{simulation_name}.pt")
    ...
    print(f"Finished training GRU-ODE for Climate. Saved in ./trained_models_atten/{simulation_name}")
...
if __name__ =="__main__":
    ...
    # import pdb 
    # pdb.set_trace()
    simulation_name="USHCN_IJICAI_"+str(args.seed)+'_'+str(args.delta_t)+'_'+str(args.lr)+"_"+str(args.p_hidden)+"_"+str(args.prep_hidden)
    device = torch.device("cuda")
    # import pdb
    # pdb.set_trace()
    torch.random.manual_seed(args.seed)
    # train_idx = np.load("/home/bigdyl/gru_ode_bayes/gru_ode_bayes/datasets/Climate/folds/small_chunk_fold_idx_0/train_idx.npy",allow_pickle=True)
    # val_idx = np.load("/home/bigdyl/gru_ode_bayes/gru_ode_bayes/datasets/Climate/folds/small_chunk_fold_idx_0/val_idx.npy",allow_pickle=True)
    ...
    train_idx = np.load(DATA_PATH+"/gru_ode_bayes/datasets/Climate/folds/small_chunk_fold_idx_0/train_idx.npy",allow_pickle=True)
    val_idx = np.load(DATA_PATH+"/gru_ode_bayes/datasets/Climate/folds/small_chunk_fold_idx_0/val_idx.npy",allow_pickle=True)
    test_idx = np.load(DATA_PATH+"/gru_ode_bayes/datasets/Climate/folds/small_chunk_fold_idx_0/test_idx.npy",allow_pickle=True)
    

    #Model parameters.
    params_dict=dict()
    # params_dict["csv_file_path"] = "/home/bigdyl/gru_ode_bayes/gru_ode_bayes/datasets/Climate/small_chunked_sporadic.csv" 
    

    params_dict["csv_file_path"] = DATA_PATH+"/gru_ode_bayes/datasets/Climate/small_chunked_sporadic.csv" 
    params_dict["csv_file_tags"] = None
    params_dict["csv_file_cov"]  = None
    ########## hyperparameter 변경 
    params_dict["hidden_size"] = args.hidden_size
    params_dict["p_hidden"] = args.p_hidden ##  
    params_dict["prep_hidden"] = args.prep_hidden  ## 
    params_dict["logvar"] = True
    params_dict["mixing"] = 1e-4 #Weighting between KL loss and MSE loss.
    params_dict["delta_t"]=args.delta_t

    params_dict["T"]=200
    params_dict["lambda"] = 0 #Weighting between classification and MSE loss.

    params_dict["classification_hidden"] = 2
    params_dict["cov_hidden"] = args.hidden_size
    params_dict["weight_decay"] = 0.0001 
    params_dict["dropout_rate"] = 0.2 # dropout : 0.2 
    params_dict["lr"]= args.lr
    params_dict["full_gru_ode"] = True
    params_dict["no_cov"] = True
    params_dict["impute"] = False
    params_dict["verbose"] = args.verbose #from 0 to 3 (highest)

    params_dict["T_val"] = 150
    params_dict["max_val_samples"] = 3
    params_dict["batch"] = args.batch
