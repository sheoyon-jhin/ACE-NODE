...
def parse_datasets(args, device):
    ...
    if dataset_name == "CharacterTrajectory":
        train_X = torch.load(ROOT_PATH + "/0UEA_base/train_X.pt")
        val_X = torch.load(ROOT_PATH + "/0UEA_base/val_X.pt")
        test_X = torch.load(ROOT_PATH + "/0UEA_base/test_X.pt")
        train_y = torch.load(ROOT_PATH + "/0UEA_base/train_y.pt")
        val_y = torch.load(ROOT_PATH + "/0UEA_base/val_y.pt")
        test_y = torch.load(ROOT_PATH + "/0UEA_base/test_y.pt")
        train_data = []
        val_data = []
        test_data = []
    ...
    if dataset_name == "CharacterTrajectory30":
        train_X = torch.load(ROOT_PATH + "/30UEA_base/train_X.pt")
        val_X = torch.load(ROOT_PATH + "/30UEA_base/val_X.pt")
        test_X = torch.load(ROOT_PATH + "/30UEA_base/test_X.pt")
        train_y = torch.load(ROOT_PATH + "/30UEA_base/train_y.pt")
        val_y = torch.load(ROOT_PATH + "/30UEA_base/val_y.pt")
        test_y = torch.load(ROOT_PATH + "/30UEA_base/test_y.pt")
    ...
    if dataset_name == "CharacterTrajectory50":
        ...
        train_X = torch.load(ROOT_PATH + "/50UEA_base/train_X.pt")
        val_X = torch.load(ROOT_PATH + "/50UEA_base/val_X.pt")
        test_X = torch.load(ROOT_PATH + "/50UEA_base/test_X.pt")
        train_y = torch.load(ROOT_PATH + "/50UEA_base/train_y.pt")
        val_y = torch.load(ROOT_PATH + "/50UEA_base/val_y.pt")
        test_y = torch.load(ROOT_PATH + "/50UEA_base/test_y.pt")
        # import pdb;pdb.set_trace()
        # if torch.isnan(
    ...
    if dataset_name == "CharacterTrajectory70":
        train_X = torch.load(ROOT_PATH + "/70UEA_base/train_X.pt")
        val_X = torch.load(ROOT_PATH + "/70UEA_base/val_X.pt")
        test_X = torch.load(ROOT_PATH + "/70UEA_base/test_X.pt")
        train_y = torch.load(ROOT_PATH + "/70UEA_base/train_y.pt")
        val_y = torch.load(ROOT_PATH + "/70UEA_base/val_y.pt")
        test_y = torch.load(ROOT_PATH + "/70UEA_base/test_y.pt")
        # import pdb;pdb.set_trace()
        # if torch.isnan(
    ...
    if dataset_name == "stock":
        ...
        data_path = ROOT_PATH + "/datasets/stock_data.csv"
    ...
    if dataset_name == "stock30":
        ...
        data_x, data_y = torch.Tensor(data_x), torch.Tensor(data_y)
        generator = torch.Generator().manual_seed(56789)

        for data in data_x:
            removed_points = (
                torch.randperm(seq_len, generator=generator)[: int(seq_len * 0.3)]
                .sort()
                .values
            )
            data[removed_points] = float("nan")
    ...
    if dataset_name == "stock50":
        ...
        for data in data_x:
            removed_points = (
                torch.randperm(seq_len, generator=generator)[: int(seq_len * 0.5)]
                .sort()
                .values
            )
            data[removed_points] = float("nan")
    ...
    if dataset_name == "stock70":
        ...
        for data in data_x:
            removed_points = (
                torch.randperm(seq_len, generator=generator)[: int(seq_len * 0.7)]
                .sort()
                .values
            )
            data[removed_points] = float("nan")
    ...
    if dataset_name == "sepsisio":
        # import pdb ; pdb.set_trace()
        train_X = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/IO/train_X.pt"
        )
        val_X = torch.load("/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/IO/val_X.pt")
        test_X = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/IO/test_X.pt"
        )
        train_y = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/IO/train_y.pt"
        )
        val_y = torch.load("/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/IO/val_y.pt")
        test_y = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/IO/test_y.pt"
        )
        train_data = []
        val_data = []
        test_data = []
    ...
    if dataset_name == "sepsisiono":
        # import pdb ; pdb.set_trace()
        train_X = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/NOIO/train_X.pt"
        )
        val_X = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/NOIO/val_X.pt"
        )
        test_X = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/NOIO/test_X.pt"
        )
        train_y = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/NOIO/train_y.pt"
        )
        val_y = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/NOIO/val_y.pt"
        )
        test_y = torch.load(
            "/home/bigdyl/socar/ACE_NODE/latent_ode/0SEPSIS/NOIO/test_y.pt"
        )
        train_data = []
        val_data = []
        test_data = []
...
# CUDA_VISIBLE_DEVICES=2 python3 run_models.py --random-seed 11 --niters 30 -n 8000 -l 20 --dataset stock --attentive-ode --rec-dims 40 --rec-layers 3 --gen-layers 3 --units 50 --gru-units 50 --quantization 0.016 --extrap --b 200
