...
# from lib.create_attentive_latent_ode_model import create_Attentive_LatentODE_model
from lib.attentive_latent_ode_model import create_Attentive_LatentODE_model
...
parser.add_argument('--attentive-ode', action='store_true', help="Run Latent ODE seq2seq model")
...
if __name__ == '__main__':
	...
	ckpt_path = os.path.join(args.save, "seed_"+str(args.random_seed)+"experiment_fc_obs_" + str(experimentID) + '.ckpt')
	...
	obsrv_std = torch.Tensor([obsrv_std]).to(device) #0.0100
	...
	if args.rnn_vae:
		...
		model = create_LatentODE_model(args, input_dim, z0_prior, obsrv_std, device, 
			classif_per_tp = classif_per_tp,
			n_labels = n_labels)
	elif args.attentive_ode:
		
		model = create_Attentive_LatentODE_model(args, input_dim, z0_prior, obsrv_std, device, 
			classif_per_tp = classif_per_tp,
			n_labels = n_labels)
	...
	log_path = "logs/" + file_name + "_fcobs_" + str(experimentID) + ".log"
				...
				with torch.no_grad():
					...
					if isinstance(model, LatentODE) and (args.dataset == "periodic"): #and not args.classic_rnn and not args.ode_rnn:
						...
						viz.draw_all_plots_one_dim(test_dict, model, 
							plot_name = file_name + "_fc_obs" + str(experimentID) + "_{:03d}".format(plot_id) + ".png",
